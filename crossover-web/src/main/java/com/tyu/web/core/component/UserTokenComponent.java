package com.tyu.web.core.component;

import java.io.IOException;
import java.util.concurrent.TimeUnit;

import com.tyu.common.util.Des;
...
public class UserTokenComponent {
	...
	public UserPrincipalVO createToken(UserPrincipalVO userPrincipalVO) {
		if (null == userPrincipalVO || StringUtils.isBlank(userPrincipalVO.getUsername())) {
			throw new NullPointerException("创建token失败,对象信息不能为空");
		}
		String key = PRE.concat(userPrincipalVO.getUsername());
		redisTemplate.delete(key);
		ValueOperations<Object, Object> operations = redisTemplate.opsForValue();
		userPrincipalVO.setToken(Des.encode(System.currentTimeMillis() + "|" + userPrincipalVO.getUsername()));
